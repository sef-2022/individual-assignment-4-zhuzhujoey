...
public class Service {
    ...
    public boolean addCustomerToService(Customer customer, Franchise fID)  {
        if (customer.getFranchise().getServices().size() < 3) { // check if the customer has less than 3 services
            for ( int i = 0; i < customer.getFranchise().getServices().size(); i++) {
                if (fID.getServices().get(i).getCustomer().contains(customer)) {
                    System.out.println("Customer already enrolled in a service");
                    return false;
                    // check if the customer is already enrolled in a service

                }
            }
            } else {
                if (fID.getCustomer().size() < 500) {
                    customer.getFranchise().getServices().add(this);
                    fID.getCustomer().add(customer);
                    customer.setNumberOfServices(customer.getNumberOfServices() + 1);
                    // check if the franchise has less than 500 customers
                    return true;
                }
            }
        return false;
